...
hex_code: 7zty55
title: "Three Skills Shipped, Two Date Families Untangled, and a Repo That Can Keep a Secret"
lede: "A skill library that publishes everything needs somewhere to put the things it can't. This session built that boundary — gitignored config read by instruction, an audit grep that has to come back empty, and a rule against injecting our conventions into repos we only have READ on — then shipped three skills through it and finally wrote down the frontmatter standard the tree had been running on tacitly for months."
publish: true
