/**
 * Minimal frontmatter splitter + YAML subset parser. Scoped to the shapes
 * authored in Lossless changelog/ and SKILL.md frontmatter — not a general
 * YAML parser. Astro Knots: "fewer dependencies is always better."
 *
 * Supports: top-level key:value, quoted strings, block-style arrays
 * (`key:` followed by indented `  - item`), flow-style arrays (`[a, b, c]`),
 * booleans, numbers, unquoted scalars (left as strings — zod schemas coerce).
 *
 * Does NOT support nested mappings, anchors, aliases, tags, or block scalars
 * (`|`, `>`). Lines that fail to parse are skipped, not thrown — rejecting a
 * whole entry over one stray quirk would defeat the loader's "respect what's
 * there" stance.
 */
