/**
 * /llms-full.txt — concatenated raw markdown of every skill body and published
 * changelog entry. Preferred ingest target for LLMs that can handle a single
 * large document (avoids per-page HTTP overhead).
 * Spec: https://llmstxt.org/
 *
 * Template lives at `src/llms/llms-full.md`. Publish gate matches the rendered
 * pages exactly: `data.publish !== false`.
 */
...
export const GET: APIRoute = async () => {
  ...
  function appendEntry(
    ...
    collectionLabel: 'skill' | 'changelog',
    canonicalUrl: string,
  ): void {
  ...
  for (const entry of skills) {
    appendEntry(entry as any, 'skill', `${root}/skills/${entry.id}/`);
  }
