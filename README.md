...
| **Archify** *(submodule)* | [`archify/`](./archify/) | Upstream skill from [`tt-a1i/archify`](https://github.com/tt-a1i/archify) (MIT) that turns a small typed JSON spec into a validated, self-contained HTML diagram with inline SVG — `architecture` / `workflow` / `sequence` / `dataflow` / `lifecycle` types, dual themes, PNG / JPEG / WebP / SVG / WebM export, and pasted Mermaid accepted as input. Pinned as a git submodule for the same reason as Chroma's. **Two unrelated projects ship as "Archify"** — this is the CLI / agent-skill one, *not* `Harrison-Yuan/archify-webui`; our `self-host-stack` watchlist had flagged the ambiguity and left it open. Worth vendoring because it answers the standing objection to natural-language diagramming (an LLM drawing a plausible picture is a hand-drawn diagram with extra steps): `--repo-root` makes the architecture type inspect **repository evidence** rather than a description, and `compare … --receipt` diffs two specs so a diagram announces its own staleness instead of rotting quietly. Validation is adversarial — it returns coordinate-level geometry proofs and refuses delivery until they clear. Needs a hand-made symlink into `~/.claude/skills` (the skill sits at `archify/archify/SKILL.md`, one level deeper than `sync-skills-symlinks.sh` scans), exactly as `chroma-local` / `chroma-cloud` do. |
...
| **Create Contact List from Image or Spreadsheet** | [`create-contact-list-from-image-or-spreadsheet/`](./create-contact-list-from-image-or-spreadsheet/) | Turns a roster in whatever form it arrived — phone photo of a printed sign-in sheet, screenshot, PDF, CSV, spreadsheet — into a verified contact CSV, vCard 3.0 files importable on both iOS and Android, and a paste-ready E.164 number list. The centre of it is the **band-crop transcription discipline**, not the vCard generation: never read digits off a downscaled overview (it is legible enough to *look* right and wrong often enough to poison a roster), crop into overlapping full-resolution bands so a boundary misread reveals itself, and auto-orient first because **EXIF orientation lies**. A single wrong digit means a stranger in a private group chat. Encodes E.164-always (a formatted `TEL` matches a messaging account only *sometimes*, which is worse than never), the fact that vCard 3.0 alone covers iOS / Android / macOS / Outlook so a 4.0 twin adds no reach, the WhatsApp name-prefix trick (its group picker searches contact **names only**), flag-don't-fix verification, and the observation that `numbers.txt` is a **consumable, not a record** — it empties as the user works through it, so a blank file is a finished job rather than corruption. Privacy is structural: deliverables land in `~/Downloads/<group>/`, never a working tree, never `context-v/`. `scripts/build-contacts.py` is stdlib-only. |
